...
def post_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            search_vector = SearchVector('title', weight='A') + SearchVector('body', weight='B') # Weighting queries. A=1.0 B=0.4
            search_query = SearchQuery(query)
            results = Post.published.annotate(
                search=search_vector, 
                rank=SearchRank(search_vector, search_query)
                                              ).filter(rank__gte=0.3).order_by('-rank') # Filter results by rank higher than 0.3
